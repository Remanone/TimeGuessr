...
function loadRounds(): array {
    global $pdo;
    $stmt = $pdo->query('SELECT id, title, description, year, lat, lng, image, category FROM rounds ORDER BY id');
...
if ($loggedIn && $_SERVER['REQUEST_METHOD'] === 'POST' && verifyCsrf()) {
    ...
    if ($action === 'add') {
        ...
        if (!$title || !$desc || $year < 1970 || $year > 2025) {
            ...
            if (move_uploaded_file($_FILES['image']['tmp_name'], $destPath)) {
                $stmt = $pdo->prepare('INSERT INTO rounds (title, description, year, lat, lng, image, category) VALUES (?, ?, ?, ?, ?, ?, ?)');
                $stmt->execute([$title, $desc, $year, $lat, $lng, 'images/' . $imgName, $cat]);
    ...
    if ($action === 'delete') {
        ...
        $stmt = $pdo->prepare('SELECT image FROM rounds WHERE id = ?');
        $stmt->execute([$delId]);
        $row = $stmt->fetch();
        if ($row) {
            $imgPath = __DIR__ . '/' . $row['image'];
            if (file_exists($imgPath)) unlink($imgPath);
            $pdo->prepare('DELETE FROM rounds WHERE id = ?')->execute([$delId]);
            $msg = 'Round supprimé.';
            $msgType = 'success';
        } else {
    ...
    if ($action === 'edit') {
        ...
        $title = trim($_POST['title'] ?? '');
        $desc  = trim($_POST['description'] ?? '');
        $year  = max(1970, min(2025, (int)($_POST['year'] ?? 2000)));
        $lat   = (float)($_POST['lat'] ?? 0);
        $lng   = (float)($_POST['lng'] ?? 0);
        $cat   = $_POST['category'] ?? 'villes';
        if (!in_array($cat, CATEGORIES)) $cat = 'villes';
        ...
        if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
            if ($_FILES['image']['size'] <= MAX_IMAGE_SIZE && in_array($_FILES['image']['type'], ALLOWED_TYPES)) {
                ...
                if (move_uploaded_file($_FILES['image']['tmp_name'], $destPath)) {
                    // Remove old image
                    $stmtOld = $pdo->prepare('SELECT image FROM rounds WHERE id = ?');
                    ...
                    $newImage = 'images/' . $imgName;
                }
            }
        }

        if ($newImage) {
            $stmt = $pdo->prepare('UPDATE rounds SET title=?, description=?, year=?, lat=?, lng=?, category=?, image=? WHERE id=?');
            $stmt->execute([$title, $desc, $year, $lat, $lng, $cat, $newImage, $editId]);
        } else {
            $stmt = $pdo->prepare('UPDATE rounds SET title=?, description=?, year=?, lat=?, lng=?, category=? WHERE id=?');
            $stmt->execute([$title, $desc, $year, $lat, $lng, $cat, $editId]);
        }
        $msg = "Round modifié : $title";
        $msgType = 'success';
    }
}
