...
function showSummary() {
  ...
  submitScoreToServer(state.totalScore, state.mode, state.difficulty, state.results);
}
...
function submitScoreToServer(score, mode, difficulty, results) {
  const roundsData = results.map(r => ({
    round_id: r.round.id,
    guess_lat: r.guessLatLng.lat,
    guess_lng: r.guessLatLng.lng,
    guess_year: r.guessYear,
    location_score: r.locationScore,
    date_score: r.dateScore,
    total: r.totalRoundScore,
  }));
  fetch('submit_score.php', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ player_name: 'Anonyme', score, mode, difficulty, rounds_data: roundsData }),
  }).catch(() => {});
}
